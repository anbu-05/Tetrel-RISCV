module simple_mem_axi_adapter #(
    ...
    //simple_mem interface (same as Native PicoRV32 memory interface)
    ...
    always_ff @( posedge clk or negedge resetn ) begin : read_pending
        if (!resetn) begin 
            read_pending <= 0;
    ...
    always_ff @( posedge clk or negedge resetn ) begin : aw_complete
        if (!resetn) begin 
            aw_complete <= 0;
    ...
    always_ff @( posedge clk or negedge resetn ) begin : w_complete
        if (!resetn) begin 
            w_complete <= 0;
